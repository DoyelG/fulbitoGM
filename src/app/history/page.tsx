...
export default function HistoryPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {open && <RecordModal onClose={() => setOpen(false)} onSave={(m) => { addMatch(m); setOpen(false) }} />}
    </div>
  )
}
  ...
  const stats = useMemo(() => {
    ...
    for (const m of matches) {
      const process = (team: 'A' | 'B') => (p: { id: string, name: string, goals: number, performance: number }) => {
        ...
        if (team === 'A') {
          if (a > b) map[p.id].wins++; else if (a < b) map[p.id].losses++; else map[p.id].draws++
        } else {
          if (b > a) map[p.id].wins++; else if (b < a) map[p.id].losses++; else map[p.id].draws++
        }
      }
...
function RecordModal({ onClose, onSave }: { onClose: () => void, onSave: (m: Omit<Match, 'id'>) => void }) {
  ...
  const proceedToScoring = () => {
    if (teamA.length !== playersPerTeam) { alert(`Team A needs exactly ${playersPerTeam} players.`); return }
    if (teamB.length !== playersPerTeam) { alert(`Team B needs exactly ${playersPerTeam} players.`); return }
  }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-4">
        ...
        <div className="grid md:grid-cols-3 gap-3 mb-4">
          <DropCol title="Available Players" onDrop={e => onDrop(e, 'unassigned')}>
            {unassigned.map(p => <Draggable key={p.id} p={p} />)}
          </DropCol>
          <DropCol title={`Team A (${teamA.length}/${playersPerTeam})`} onDrop={e => onDrop(e, 'a')}>
            {teamA.map(p => <Draggable key={p.id} p={p} />)}
          </DropCol>
          <DropCol title={`Team B (${teamB.length}/${playersPerTeam})`} onDrop={e => onDrop(e, 'b')}>
            {teamB.map(p => <Draggable key={p.id} p={p} />)}
          </DropCol>
        </div>

        <div className="flex gap-3 justify-center mb-6">
          <button className="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700" onClick={() => { setTeamA([]); setTeamB([]) }}>
            Reset Teams
          </button>
          <button className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" onClick={proceedToScoring}>
            Proceed to Scoring
          </button>
        </div>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-3">
          <div className="bg-gray-50 rounded p-3">
            <h4 className="text-center font-semibold mb-2">Team A</h4>
            <input type="number" min={0} placeholder="Team Goals" value={teamAScore} onChange={e => setTeamAScore(e.target.value === '' ? '' : Number(e.target.value))} className="w-24 text-center border rounded px-2 py-1 mb-2" />
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {teamA.map(p => (
                <div key={p.id} className="flex items-center justify-between bg-white border rounded px-3 py-2">
                  <span className="font-medium">{p.name}</span>
                  <div className="flex items-center gap-2">
                    <input type="number" min={0} max={20} value={goalsA[p.id] ?? 0} onChange={e => setGoalsA(s => ({ ...s, [p.id]: Number(e.target.value || 0) }))} className="w-16 text-center border rounded px-2 py-1" />
                    <input type="number" min={1} max={10} step={0.1} value={perfA[p.id] ?? 5} onChange={e => setPerfA(s => ({ ...s, [p.id]: Number(e.target.value || 5) }))} className="w-16 text-center border rounded px-2 py-1" />
                  </div>
                </div>
              ))}
            </div>
            <div className={`mt-2 text-center font-semibold rounded px-2 py-1 ${teamAScore === totalGoalsA ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          ...
          <div className="bg-gray-50 rounded p-3">
            <h4 className="text-center font-semibold mb-2">Team B</h4>
            <input type="number" min={0} placeholder="Team Goals" value={teamBScore} onChange={e => setTeamBScore(e.target.value === '' ? '' : Number(e.target.value))} className="w-24 text-center border rounded px-2 py-1 mb-2" />
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {teamB.map(p => (
                <div key={p.id} className="flex items-center justify-between bg-white border rounded px-3 py-2">
                  <span className="font-medium">{p.name}</span>
                  <div className="flex items-center gap-2">
                    <input type="number" min={0} max={20} value={goalsB[p.id] ?? 0} onChange={e => setGoalsB(s => ({ ...s, [p.id]: Number(e.target.value || 0) }))} className="w-16 text-center border rounded px-2 py-1" />
                    <input type="number" min={1} max={10} step={0.1} value={perfB[p.id] ?? 5} onChange={e => setPerfB(s => ({ ...s, [p.id]: Number(e.target.value || 5) }))} className="w-16 text-center border rounded px-2 py-1" />
                  </div>
                </div>
              ))}
            </div>
            <div className={`mt-2 text-center font-semibold rounded px-2 py-1 ${teamBScore === totalGoalsB ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button className={`px-4 py-2 rounded text-white ${canSave ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'}`} disabled={!canSave} onClick={handleSave}>
            Save Match
          </button>
        </div>
      </div>
...
function DropCol({ title, onDrop, children }: { title: string, onDrop: (e: React.DragEvent<HTMLDivElement>) => void, children: React.ReactNode }) {
  return (
    <div className="border-2 border-dashed rounded p-3 min-h-48" onDragOver={e => e.preventDefault()} onDrop={onDrop}>
