  ...
  const sorted = useMemo(() => {
    const getValue = (row: StatRow, key: SortKey) => {
      if (key === "goalsPerMatch")
        return row.matches === 0 ? 0 : row.goals / row.matches;
      if (key === "winRate")
          ...
          <table className="min-w-full table-auto">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  className="px-4 py-3 text-left text-sm font-semibold text-gray-700 cursor-pointer select-none"
                  onClick={() => toggleSort("totalPerformance")}
                >
                  Rendimiento
                  ...
                  return (
                    <tr key={stat.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        {(stat.goals / stat.matches).toFixed(2)}
                      </td>
                      <td className="px-4 py-3">
